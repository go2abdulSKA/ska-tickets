{{-- 
    resources/views/livewire/masters/department/delete-department.blade.php
    
    Delete Department Confirmation Modal
    Shows confirmation dialog before deleting a department
--}}
        ...
        <div class="modal-content">
            ...
            <div class="p-4 text-center modal-body">
                ...
                @if ($departmentToDelete)
                    <div class="mb-4 alert alert-warning text-start">
                        ...
                        <div class="gap-3 mb-3 d-flex align-items-start">
                            {{-- Logo --}}
                            @if($departmentToDelete->logo_path)
                                <img src="{{ asset('storage/' . $departmentToDelete->logo_path) }}" 
                                     alt="{{ $departmentToDelete->department }}"
                                     class="rounded"
                            ...
                            <div class="flex-grow-1">
                                <h6 class="mb-1">{{ $departmentToDelete->department }}</h6>
                                @if($departmentToDelete->short_name)
                                    <p class="mb-1 text-muted small">
                                        <strong>Short Name:</strong> {{ $departmentToDelete->short_name }}
                                    </p>
                                @endif
                                <p class="mb-0 text-muted small">
                                    <strong>Prefix:</strong> 
                                    <span class="badge badge-soft-primary">{{ $departmentToDelete->prefix }}</span>
                                </p>
                            </div>
                        </div>
                        ...
                        {{-- If department is in use, show warning --}}
                        @if($usageCount > 0)
                ...
                <div class="gap-2 d-grid">
                    @if($departmentToDelete && $usageCount == 0)
                        {{-- Confirm Delete Button (only if not in use) --}}
                        <button wire:click="delete" class="btn btn-danger btn-lg">
                            <i class="ti ti-trash me-1"></i> Yes, Delete It!
                        </button>
                    @else
                        {{-- Deactivate Button (if in use) --}}
